...
To start developing, run the following from your terminal:
...
<Note>
  ...
  <GenericLink href="/download">Now Desktop</GenericLink>, which is the easiest
  way to get started with Now. Alternatively, you can install Now CLI with Yarn
  using the following in your terminal:{' '}
  <InlineCode>yarn global add now</InlineCode>.
</Note>
...
When complete, Now CLI will provide you with the URL your project has been deployed and aliased to. In the case above, we asked to alias to `my-mongodb-api.now.sh`.
...
See more on our Git integrations:
